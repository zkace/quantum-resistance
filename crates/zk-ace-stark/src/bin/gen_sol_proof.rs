...
//! Outputs JSON with `proof_hex`, `proof_size`, and `public_inputs`.
//!
...
fn main() {
    ...
    let (tx_hash, nonce_val) = if args.len() > 1 {
        ...
        let nonce_val: u64 = args.get(2).and_then(|s| s.parse().ok()).unwrap_or(0);
        ...
        let p = 18_446_744_069_414_584_321u64;
        let th = [
            B::new(u64::from_be_bytes(hash[0..8].try_into().unwrap()) % p),
            B::new(u64::from_be_bytes(hash[8..16].try_into().unwrap()) % p),
            B::new(u64::from_be_bytes(hash[16..24].try_into().unwrap()) % p),
            B::new(u64::from_be_bytes(hash[24..32].try_into().unwrap()) % p),
        ];
        eprintln!("Using calldata txHash: [{}, {}, {}, {}]",
            th[0].as_int(), th[1].as_int(), th[2].as_int(), th[3].as_int());
        (th, nonce_val)
    } else {
        ([B::new(0xAAAAu64), B::new(0xBBBBu64), B::new(0xCCCCu64), B::new(0xDDDDu64)], 0u64)
    };
    ...
    eprintln!("Context elements ({}):", context_elems.len());
    for (i, e) in context_elems.iter().enumerate() {
        eprintln!("  [{i}] {} (0x{:016x})", e.as_int(), e.as_int());
    }
    let mut coin_seed: Vec<B> = context_elems;
    coin_seed.append(&mut public_inputs.to_elements());
    eprintln!("Total seed elements: {}", coin_seed.len());
    ...
    // Layout in proof blob: trace_current[0..18], trace_next[18..36],
    //                       constraint_current[36], constraint_next[37]
    for e in &ood_trace_current {
        encode_ext_element(&mut abi, *e);
    }
    for e in &ood_trace_next {
        encode_ext_element(&mut abi, *e);
    }
    for e in &ood_constraint_current {
        encode_ext_element(&mut abi, *e);
    }
    for e in &ood_constraint_next {
        encode_ext_element(&mut abi, *e);
    }
    ...
    //   constraintLeaf   32
    //   tracePath      6×32 = 192
    //   constraintPath 6×32 = 192
    //   traceEvals    18×32 = 576
    //   constraintEval 2×32 =  64
    //   total              = 1120
    for (qi, &pos) in unique_pos.iter().enumerate() {
        ...
        // constraintLeaf (32 bytes)
        abi.extend_from_slice(&constraint_leaves[qi].as_bytes());

        // tracePath (6 siblings)
        let tp = &trace_openings[qi].1;
        for node in tp.iter() {
            abi.extend_from_slice(&node.as_bytes());
        }
        for _ in tp.len()..MERKLE_DEPTH {
            abi.extend_from_slice(&[0u8; 32]);
        }

        // constraintPath (6 siblings)
        let cp = &constraint_openings[qi].1;
        for node in cp.iter() {
            abi.extend_from_slice(&node.as_bytes());
        }
        for _ in cp.len()..MERKLE_DEPTH {
            abi.extend_from_slice(&[0u8; 32]);
        }

        // traceEvals: 18 base elements as ABI uint256
        let trace_row = trace_states.get_row(qi);
        assert_eq!(trace_row.len(), TRACE_WIDTH, "trace row width mismatch");
        for &val in trace_row.iter() {
            encode_u64(&mut abi, val.as_int());
        }

        // constraintEval: 1 extension element = 2 ABI uint256
        let cval = constraint_evals.get_row(qi)[0];
        encode_ext_element(&mut abi, cval);

        let q_size = abi.len() - q_start;
        assert_eq!(q_size, QSZ, "query {} size: got {}, expected {}", qi, q_size, QSZ);
    }

    // Verify total
    let expected_total = O_QD + unique_pos.len() * QSZ;
    assert_eq!(
        abi.len(),
        expected_total,
        "total proof size mismatch: got {}, expected {}",
        abi.len(),
        expected_total
    );
    ...
    // 7. Output JSON
    // =====================================================================
    let proof_hex = format!("0x{}", hex::encode(&abi));

    // Public inputs: 17 elements in the order expected by the Solidity verifier
    // id_com[0..4], target[0..4], rp_com[0..4], domain, tx_hash[0..4]
    let pi_vec: Vec<u64> = {
        let mut v = Vec::with_capacity(17);
        for e in &public_inputs.id_com {
            v.push(e.as_int());
        }
        for e in &public_inputs.target {
            v.push(e.as_int());
        }
        for e in &public_inputs.rp_com {
            v.push(e.as_int());
        }
        v.push(public_inputs.domain.as_int());
        for e in &public_inputs.tx_hash {
            v.push(e.as_int());
        }
        v
    };
    ...
    let output = json!({
        ...
        "num_queries": unique_pos.len(),
        "layout": {
            "O_TRACE": O_TRACE,
            "O_CONSTR": O_CONSTR,
            "O_FRI": O_FRI,
            "O_OOD_DIGEST": O_OOD_DIGEST,
            "O_POW": O_POW,
            "O_OOD": O_OOD,
            "O_Z": O_Z,
            "O_CC": O_CC,
    ...
    eprintln!("Proof size: {} bytes", abi.len());
    eprintln!("  Header:  {} bytes", O_QD);
    eprintln!(
        "  Queries: {} x {} = {} bytes",
        unique_pos.len(),
        QSZ,
